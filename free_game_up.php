...
<body>
    <form action = "free_game_up.php" method = "post" class = 'upload-form'>
        ...
        <input type = "number" name = "frequency" placeholder="AD Frequency"/><br><br>
...
<?php
    if(!empty($_POST["game_name"]) && !empty($_POST["img_url"]) && !empty($_POST["genre"]) && !empty($_POST["frequency"])) {
        ...
        $sql = "INSERT INTO free_games
                (game_id, ad_frequency)
                VALUES
                ('{$game_id}', '{$_POST["frequency"]}')";
        $result = mysqli_query($conn, $sql);
